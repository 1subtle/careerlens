...
> 修订日期：2026 年 9 月 7 日。以本目录的《T5 实训要求整理》为需求依据，按 4—5 人团队、9 月 12 日 12:00 前提交安排范围。本文确定产品与技术方案，具体任务见[构建计划](/Users/nresearch/Desktop/careerlens/CareerLens_构建计划.md)。
...
该固定基线已导入并在本机运行，依赖锁文件已提交。首版实现与验证结果见 [README](README.md) 和 [验证记录](docs/验证记录.md)；Docker 与真实模型验收仍待完成。
...
首版按本地单人工作区交付，沿用上游一个主简历的模型。3 份学生简历分别作为隔离的评估案例，不混成同一个人的版本。用户的简历正文、联系方式和 API Key 不进入普通日志；模型调用只发送当前分析所需内容，演示使用脱敏数据。
...
最小市场模块补充岗位样本层面的技能与薪资观察。演示遵循“输入材料—解释差距—依据事实修改—比较结果—查看岗位样本”的顺序。开发任务、时间节点和验收标准见[CareerLens 构建计划](/Users/nresearch/Desktop/careerlens/CareerLens_构建计划.md)。
