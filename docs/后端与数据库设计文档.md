...
文本和 PDF/DOCX/TXT/Markdown 导入先返回待校对草稿；用户保存后才创建 CareerLens 简历。文件上限 5 MiB，解析文本最多 30,000 字符；扫描件未实现 OCR。除积分异常外，文件 AI 解析失败可返回规则草稿、原文及 `warning`，纯文本 AI 执行失败返回 502。积分异常保留原状态码，不降级。
...
没有有效要求时返回 `null`。规则版本为 `career-1.1`。每项取证据支持的最高值，同一技能重复出现不重复加权；学历、经验和到岗条件单列，不计入 $S$。人工确认 `supported` 至少引用一条经历证据。规则数值反映材料支持程度。
