...
当前规则版本为 `career-1.1`。对第 $i$ 项要求，权重记为 $w_i$，证据值记为 $v_i$，贡献记为 $c_i$：
...
文本解析和 JD 正文为 1–30,000 字符；空白输入无效。单份文件上限 5 MiB。JD 要求最多 60 项，ID 和名称不得重复，`source_text` 必须来自 JD 原文。比较必须选择 2–5 个不同岗位；补充事实最多 8 项、每项最多 1,000 字符。本页分析最多 10 份完整 JD。实时检索页码为 1–100，关键词最多 100 字符，提供商和地区采用枚举校验。
